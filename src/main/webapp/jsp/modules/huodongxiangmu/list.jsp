...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">活动项目管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>活动项目管理</span></li>
								<li class="breadcrumb-item active"><span>活动项目列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">活动项目列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										活动
									</label>
									<input type="text" id="huodongSearch" class="form-control form-control-sm" placeholder="" aria-controls="tableId">
									<label>
										类型
									</label>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('huodong')">活动<i id="huodongIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('leixing')">类型<i id="leixingIcon" class="fa fa-sort"></i></th>

											<th>封面</th>
											<th onclick="sort('jihedian')">集合点<i id="jihedianIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('shijian')">时间<i id="shijianIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('didian')">地点<i id="didianIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#huodongSearch').val() != null && $('#huodongSearch').val() != ''){
				searchForm.huodong ="%" +  $('#huodongSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("huodongxiangmu/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
				huodong : searchForm.huodong,	
		...
		function setDataRow(item,number){
			...
			var huodongCell  = document.createElement('td');

						 huodongCell.innerHTML = item.huodong;


			row.appendChild(huodongCell);
			...
			var jihedianCell  = document.createElement('td');

						 jihedianCell.innerHTML = item.jihedian;


			row.appendChild(jihedianCell);
			...
			var didianCell  = document.createElement('td');

						 didianCell.innerHTML = item.didian;


			row.appendChild(didianCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("huodongxiangmu/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function getSum(colName){
			http("huodongxiangmu"+colName,"GET",{
		...
		function leixingSelect(){
			//填充下拉框选项
			http("option/fenleiliebiao/leixing","GET",{},(res)=>{
				if(res.code == 0){
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"huodongxiangmu");
