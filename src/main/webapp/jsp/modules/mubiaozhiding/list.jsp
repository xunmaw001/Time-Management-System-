...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">目标制定管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>目标制定列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">目标制定列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										目标
									</label>
									<input type="text" id="mubiaoSearch" class="form-control form-control-sm" placeholder="" aria-controls="tableId">
									<label>
										类型
									</label>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('mubiao')">目标<i id="mubiaoIcon" class="fa fa-sort"></i></th>
											...
											<th onclick="sort('jihua')">计划<i id="jihuaIcon" class="fa fa-sort"></i></th>
											<th>操作</th>
										</tr>
									</thead>
									<tbody>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#mubiaoSearch').val() != null && $('#mubiaoSearch').val() != ''){
				searchForm.mubiao ="%" +  $('#mubiaoSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("mubiaozhiding/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
				mubiao : searchForm.mubiao,	
		...
		function setDataRow(item,number){
			...
			var mubiaoCell  = document.createElement('td');

						 mubiaoCell.innerHTML = item.mubiao;


			row.appendChild(mubiaoCell);
			...
			var jihuaCell  = document.createElement('td');

						  //如果图文详情包含图片
			  if(item.jihua && item.jihua.indexOf('img') != -1){
				//暂时只考虑图片+文字 和 文字+图片的情况
				var beginIndex = item.jihua.indexOf('<img');
				var endIndex = item.jihua.indexOf('>');				
				jihuaCell.innerHTML = item.jihua.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
			  }else{
			  	 if(item.jihua != null && item.jihua != "" && item.jihua.length >= 11){
			  		jihuaCell.innerHTML = item.jihua.substring(0, 10) + "..."; 
			  	}else{
			  		jihuaCell.innerHTML = item.jihua;
			 	}
			  }
				  


			row.appendChild(jihuaCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("mubiaozhiding/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"mubiaozhiding");
