    ...
    <fieldset class="search-container" style="text-align: center;margin: 50px 0;">
      <form class="layui-form" style="margin: 10px 0;">
        ...
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('huodongxiangmu','新增')" @click="jump('../huodongxiangmu/add.jsp')" type="button" class="layui-btn btn-theme">
      ...
      <div class="box">
        <ul class="clearfix">
          <li v-for="(item,index) in dataList" v-bind:key="index">
            <a :href="'../huodongxiangmu/detail.jsp?id='+item.id">
              <figure>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#huodong').val()) {
          param['huodong'] = jquery('#huodong').val() ? '%' + jquery('#huodong').val() + '%' : '';
        }  
        if (jquery('#leixing').val()) {
          param['leixing'] = jquery('#leixing').val() ? '%' + jquery('#leixing').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('huodongxiangmu/list', 'get', param, function(res) {
